...
<head>
    <meta charset="UTF-8" />
    <title>Estimate Register</title>
    <link rel="icon" href="{{ asset('img/favicon.png') }}">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <meta name="base-url" content="{{ url('/') }}">
    <meta name="customer-id" content="{{ $customer->ID }}">
    <meta name="estimate-id" content="{{ $estimateId ?? '' }}">
    ...
    <link rel="stylesheet" href="{{ asset('css/estimate_register.css') }}">
...
<body>
    <div id="main-container">
        ...
        <section id="dash">
                ...
                <div class="right-info">
                    ...
                    <div class="customer-box">
                        ...
                        <div class="policy-select-wrap">
                            ...
                            <div class="estimate-number-wrap">
                                <label class="estimate-number-label">Estimate #</label>
                                <input id="estimateNumberBox" class="estimate-number-box" type="text"
                                    value="{{ $estimateNumber ?? '' }}" placeholder="EST-0000" readonly>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="table-card" data-save-url="{{ route('estimates.table.save', ['customerId' => $customerId]) }}"
                data-estimates-url="{{ route('estimates', ['customerId' => $customerId]) }}">

                <button id="btnSaveEstimateTable" class="btn-save-table">Save</button>

                <div class="charges-box"
                    data-save-url="{{ route('estimates.charges.save', ['customerId' => $customerId]) }}">
                        ...
                        <div id="premiumSplitFields" class="split-fields"
                            ...
                            <div class="split-block">
                                ...
                                <select id="premiumP1Method" class="charge-select">
                                    <option value="">Select</option>
                                    <option value="Cash"
                                        {{ ($premiumP1Method ?? '') === 'Cash' ? 'selected' : '' }}>Cash</option>
                                    <option value="Credit/Debit Card"
                            ...
                            <div class="split-block">
                                ...
                                <select id="premiumP2Method" class="charge-select">
                                    <option value="">Select</option>
                                    <option value="Cash"
                                        {{ ($premiumP2Method ?? '') === 'Cash' ? 'selected' : '' }}>Cash</option>
                                    <option value="Credit/Debit Card"
                ...
                <div class="invoice-dates"
                    data-save-url="{{ route('estimates.dates.save', ['customerId' => $customerId]) }}">
                ...
                <div class="table-topbar">
                    <button id="btnAddEstimateRow" class="btn-add-row">Add Row</button>
                    <div class="grand-total">Total: <span id="estimateGrandTotal">$0.00</span></div>
                </div>

                <div class="table-scroll">
                    <datalist id="estimateItemOptions">
                    ...
                    <table class="invoice-table" id="estimateTable">
                        <thead>
                            <tr>
                                <th>Item</th>
                                <th>Amount</th>
                                        ...
                                        <td>
                                            <div class="item-wrap">
                                                <input class="cell-input item-input" type="text"
                                                    list="estimateItemOptions" value="{{ $r['item'] ?? '' }}">
                                                <span class="item-arrow"></span>
                                            </div>
                                        </td>
                                        <td><input class="cell-input qty-input" type="text"
                                                value="{{ $r['amount'] ?? '' }}"></td>
                                        <td><input class="cell-input price-input" type="text"
                                                value="{{ $r['price'] ?? '' }}"></td>
                                        <td class="row-total">{{ !empty($r['total']) ? '$' . $r['total'] : '$0.00' }}
                                        </td>
                                        <td class="row-actions"><button type="button" class="btn-trash"
                ...
                <div class="hint">* El total se calcula automático (Amount) y se suma al Total general.</div>
    ...
    <script src="{{ asset('js/estimate_register.js') }}"></script>
</body>
