...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/general-payments.css') }}">
    ...
    <div id="main-container">
        ...
        <section id="dash">
            ...
            <div id="lower-table-clients" type="fullscreen" class="general-payments-wrapper">
                ...
                <form method="GET" action="{{ route('payments.general') }}" class="general-payments-filters"
                    ...
                    <div class="left-filters">
                        <select name="per_page" id="perPageSelect"
                            onchange="document.getElementById('generalPaymentsForm').submit()">
                            <option value="50" {{ $perPageInput == '50' ? 'selected' : '' }}>50</option>
                            <option value="100" {{ $perPageInput == '100' ? 'selected' : '' }}>100</option>
                            <option value="200" {{ $perPageInput == '200' ? 'selected' : '' }}>200</option>
                    ...
                    <div class="right-filters">
                        <input type="text" name="search" id="liveSearchInput" value="{{ $search }}"
                            placeholder="Search by customer or date..." autocomplete="off">
                    </div>
                </form>
                        ...
                        <tbody>
                            @forelse($invoices as $invoice)
                                <tr>
                                    ...
                                    <td class="description-cell" title="{{ $invoice->description_item }}">
                                        {{ $invoice->description_item }}
                                    </td>
                                    <td>${{ number_format((float) $invoice->fee_amount, 2) }}</td>
                                    <td>${{ number_format((float) $invoice->premium_amount, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="9" class="empty-row">No invoices found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                @if ($perPageInput !== 'all' && method_exists($invoices, 'links'))
                    <div class="general-payments-pagination">
                        {{ $invoices->links() }}
                    </div>
                @endif
            </div>
...
<script>
    ...
    const tableBody = document.querySelector('.general-payments-table tbody');

    function filterPaymentsTable() {
        const searchValue = liveSearchInput.value.trim().toLowerCase();
        ...
        rows.forEach(row => {
            ...
            const matches =
                ...
                customerText.includes(searchValue);
        ...
        let noResultsRow = document.getElementById('live-search-no-results');

        if (visibleCount === 0) {
            if (!noResultsRow) {
                noResultsRow = document.createElement('tr');
                noResultsRow.id = 'live-search-no-results';
                noResultsRow.innerHTML = '<td colspan="9" class="empty-row">No invoices found.</td>';
    ...
    liveSearchInput.addEventListener('input', filterPaymentsTable);
</script>
