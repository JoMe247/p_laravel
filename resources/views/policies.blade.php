                ...
                <div class="policy-overlay-box policy-flex">
                    ...
                    <div class="policy-columns">
                        ...
                        <div class="policy-right">
                            ...
                            <div style="display:flex; flex-wrap:wrap; gap:10px; margin-left:10px; margin-bottom:10px;">
            ...
            <div id="policy-log-overlay" class="policy-log-overlay" style="display:none;">
                <div class="policy-log-box">
                    <div class="policy-log-header">
                        ...
                        <div style="display:flex; gap:10px; align-items:center;">
                            <button type="button" id="policy-log-print" class="btn">
                    ...
                    <div id="policy-log-text">
                        @forelse($policyLog as $log)
                            @php($snapshot = $log['snapshot'] ?? [])

                            <div class="policy-log-entry">
                                <div>
                                    <span>[{{ $log['created_at'] ?? '-' }}]
                                        {{ $log['action_text'] ?? 'Updated' }}</span>
                                    <strong>By:</strong> {{ $log['changed_by'] ?? 'System' }}
                                </div>

                                <div>
                                    <strong>Policy:</strong> {{ $snapshot['pol_number'] ?? '-' }}
                                    <strong>Status:</strong> {{ $snapshot['pol_status'] ?? 'Active' }}
                                </div>

                                <div><strong>EffDate:</strong> {{ $snapshot['pol_eff_date'] ?? '-' }}</div>
                                <div><strong>ExpDate:</strong> {{ $snapshot['pol_expiration'] ?? '-' }}</div>
                                <div><strong>PayDate:</strong> {{ $snapshot['pol_due_day'] ?? '-' }}</div>
                                <div><strong>Carrier:</strong> {{ $snapshot['pol_carrier'] ?? '-' }}</div>
                                <div><strong>URL:</strong> {{ $snapshot['pol_url'] ?? '-' }}</div>
                                <div><strong>Agent Record:</strong> {{ $snapshot['pol_agent_record'] ?? '-' }}</div>
                                <div><strong>Changed Fields:</strong> {{ $log['changed_fields_text'] ?? '-' }}</div>
                                <div><strong>Vehicles:</strong> {{ $log['vehicles_text'] ?? '-' }}</div>
                                <div><strong>Vehicle Changes:</strong> {{ $log['vehicle_changes_text'] ?? '-' }}</div>
                            </div>
                        @empty
                            <div class="policy-log-entry">No policy logs found.</div>
                        @endforelse
                    </div>
