(() => {
    ...
    state.shifts.forEach(s => {
      ...
      ribbon.innerHTML = `
        ...
        <div style="display:flex; gap:8px; align-items:center;">
          <button class="pencil-btn" title="Edit"><i class='bx bx-pencil'></i></button>
        </div>
      `;
  ...
  function openEditShift(shift){
    ...
    deleteShiftBtn.style.display = state.canEdit ? 'inline-flex' : 'none';
  ...
  async function deleteShift(){
    if (!state.canEdit || !state.editingShiftId) return;
    if (!confirm('Delete this shift?')) return;

    await fetch(`${baseUrl}/schedules/shifts/${state.editingShiftId}`, {
      method:'DELETE',
      credentials:'same-origin',
  ...
  function insertTimePart(v){
    // si no hay " - " aún, lo usa como inicio, si ya hay, como fin
    ...
    // si ya está completo, reemplaza el final
    if (t.includes(' - ')){
      const parts = t.split(' - ');
      timeInput.value = `${parts[0]} - ${v}`;
    } else {
      timeInput.value = `${t} - ${v}`;
    }
  }
