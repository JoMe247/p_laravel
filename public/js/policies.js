$(document).ready(function () {
    ...
    function fillMakeSelect($makeSel, selected = "") {
        ...
        COMMON_MAKES.forEach((make) => {
            $makeSel.append(`<option value="${escapeAttr(make)}">${escapeHtml(make)}</option>`);
        });
        ...
        if (selected) {
            if ($makeSel.find(`option[value="${selected}"]`).length === 0) {
                $makeSel.append(`<option value="${escapeAttr(selected)}">${escapeHtml(selected)}</option>`);
            }
    ...
    function fillYearSelect($yearSel, selected = "") {
        ...
        if (selected) {
            if ($yearSel.find(`option[value="${selected}"]`).length === 0) {
                $yearSel.append(`<option value="${escapeAttr(selected)}">${escapeHtml(selected)}</option>`);
            }
            $yearSel.val(String(selected));
        }
    }
    ...
    function loadModelsForMake($card, make, selectedModel = "", isEdit = false) {
        ...
        $.get(
            ...
            function (res) {
                ...
                const models = [
                    ...new Set(res.Results.map((x) => x.Model_Name).filter(Boolean)),
                ].sort();

                models.forEach((model) => {
                    $modelSel.append(`<option value="${escapeAttr(model)}">${escapeHtml(model)}</option>`);
                });
                ...
                if (selectedModel) {
                    if ($modelSel.find(`option[value="${selectedModel}"]`).length === 0) {
                        $modelSel.append(`<option value="${escapeAttr(selectedModel)}">${escapeHtml(selectedModel)}</option>`);
                    }
    ...
    function initYearsForCard($card) {
        fillYearSelect($card.find(".year-select"));
    }
    ...
    function createVehicleEditCardHtml(index, vehicle = {}) {
        ...
        return `
            <div class="vehicle-edit-card" data-index="${index}">
                ...
                <div class="vehicle-field">
                    <label>VIN (opcional)</label>
                    <input type="text" class="edit_vin" value="${escapeAttr(vin)}">
                </div>
    ...
    function buildVehicleSnapshot(veh) {
    ...
    function getEditVehiclesPayload() {
        let updatedVeh = [];
    ...
    function updateWebsiteButton($button, url) {
        if (!$button || !$button.length) return;

        if (isValidUrl(url)) {
            $button.attr("data-url", url).css("display", "flex");
        } else {
            $button.removeAttr("data-url").css("display", "");
        }
    }
    ...
    $(document).on("input", "#pol_number, #edit_pol_number", function () {
        ...
        if (this.id === "pol_number") {
            ...
            updateWebsiteButton($("#company-website-button"), foundUrl);
        }

        if (this.id === "edit_pol_number") {
            $("#edit_pol_carrier").val(carrier);
            $("#edit_pol_url").val(foundUrl);
            updateWebsiteButton($("#edit-company-website-button"), foundUrl);
        }
    });
    ...
    $(document).on("input change", "#pol_carrier, #edit_pol_carrier", function () {
        ...
        if (this.id === "edit_pol_carrier") {
            $("#edit_pol_url").val(foundUrl);
            updateWebsiteButton($("#edit-company-website-button"), foundUrl);
        }
    });

    $(document).on("click", "#company-website-button, #edit-company-website-button", function () {
    ...
    $("#add-vehicle-btn").on("click", function (e) {
        ...
        initYearsForCard($(`#vehicle-container .vehicle-card[data-id="${id}"]`));
    ...
    $(document).on("blur", ".vin-input", function () {
        ...
        $.get(
            ...
            function (res) {
                ...
                if (year) {
                    fillYearSelect($card.find(".year-select"), year);
                }

                if (make) {
                    fillMakeSelect($card.find(".make-select"), make);
    ...
    $(document).on("change", ".year-select", function () {
        ...
        if (year === "other") {
            $(this).hide();
            $card.find(".year-other").show().focus();
            return;
        }

        fillMakeSelect($card.find(".make-select"));
    });
    ...
    $(document).on("change", ".make-select", function () {
        ...
        if (make === "other") {
            $(this).hide();
            $card.find(".make-other").show().focus();
            $card.find(".model-select").empty().append('<option value="">Seleccione modelo</option>');
            return;
        }
    ...
    $(document).on("change", ".model-select", function () {
        ...
        if (model === "other") {
            $(this).hide();
            $card.find(".model-other").show().focus();
            return;
        }
    ...
    $(document).on("keyup change", ".year-other, .make-other, .model-other", function () {
        const $card = $(this).closest(".vehicle-card");
    ...
    $(document).on("click", ".policy-info-btn", function () {
        ...
        $.get(showUrl, function (res) {
            ...
            initialPolicySnapshot = {
                ...
                vehicules: buildVehicleSnapshot(veh),
            };

            let urlButtonValue = `<div id="edit-company-website-button"><i class='bx bx-globe'></i></div>`;

            if (isValidUrl(p.pol_url)) {
                urlButtonValue = `
                    <div id="edit-company-website-button" style="display:flex;" data-url="${escapeAttr(p.pol_url)}">
            ...
            const html = `
                <div class="edit-grid">   
                    <div class="edit-left">
                        ...
                        <input type="text" id="edit_pol_number" value="${escapeAttr(p.pol_number ?? "")}" required>

                        <label>Carrier</label>
                        <input type="text" id="edit_pol_carrier" value="${escapeAttr(p.pol_carrier ?? "")}" required>

                        <label style="position:relative;">
                            URL (Company Website)
                            ${urlButtonValue}
                        </label>
                        <input type="text" id="edit_pol_url" value="${escapeAttr(p.pol_url ?? "")}" required>

                        <label>Expiration Date</label>
                        <input type="date" id="edit_pol_expiration" value="${escapeAttr(p.pol_expiration ?? "")}" required>

                        <label>Effective Date</label>
                        <input type="date" id="edit_pol_eff_date" value="${escapeAttr(p.pol_eff_date ?? "")}" required>

                        <label>Added Date</label>
                        <input type="date" id="edit_pol_added_date" value="${escapeAttr(p.pol_added_date ?? "")}" required>

                        <label>Payment Due Day</label>
                        <input type="text" id="edit_pol_due_day" value="${escapeAttr(p.pol_due_day ?? "")}" required>

                        <label>Status</label>
                        <input type="text" id="edit_pol_status" value="${escapeAttr(p.pol_status ?? "")}" readonly required>

                        <label>Agent Record</label>
                        <input type="text" id="edit_pol_agent_record" value="${escapeAttr(p.pol_agent_record ?? "")}" required>
                    </div>
            ...
            veh.forEach((v, index) => {
                ...
                initEditCardValues($grid.find(`.vehicle-edit-card[data-index="${realIndex}"]`), v);
            });
            ...
            $overlaySave.off("click").on("click", function (e) {
                ...
                const updatedVeh = getEditVehiclesPayload();
                ...
                const currentPolicySnapshot = {
                    ...
                    pol_expiration: ($("#edit_pol_expiration").val() || "").trim(),
                    pol_eff_date: ($("#edit_pol_eff_date").val() || "").trim(),
                    pol_added_date: ($("#edit_pol_added_date").val() || "").trim(),
                    pol_due_day: ($("#edit_pol_due_day").val() || "").trim(),
                    pol_status: ($("#edit_pol_status").val() || "").trim(),
                    pol_agent_record: ($("#edit_pol_agent_record").val() || "").trim(),
                    vehicules: JSON.stringify(updatedVeh),
                };

                if (
                    initialPolicySnapshot &&
                    JSON.stringify(currentPolicySnapshot) === JSON.stringify(initialPolicySnapshot)
                ) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        pol_agent_record: currentPolicySnapshot.pol_agent_record,
    ...
    window.closeOverlayEdit = function () {
        $overlayEdit.fadeOut(150);
    };

    $(document).on("click", ".edit-status-btn", function () {
        const newStatus = $(this).data("status");
        $("#edit_pol_status").val(newStatus).trigger("change");
    });

    // =========================================================================
    //   VEHÍCULOS EN EDICIÓN
    // =========================================================================
    $(document).on("click", "#add-vehicle-btn-edit", function (e) {
        ...
        initEditCardValues($grid.find(`.vehicle-edit-card[data-index="${index}"]`), {});
    ...
    $(document).on("blur", "#policy-edit-content .edit_vin", function () {
        const vin = ($(this).val() || "").trim();
    ...
    $(document).on("change", "#policy-edit-content .edit_year_select", function () {
        const $card = $(this).closest(".vehicle-edit-card");
    ...
    $(document).on("change", "#policy-edit-content .edit_make_select", function () {
        const $card = $(this).closest(".vehicle-edit-card");
        const make = $(this).val();
    ...
    $(document).on("change", "#policy-edit-content .edit_model_select", function () {
        const $card = $(this).closest(".vehicle-edit-card");
    ...
    $(document).on(
        "keyup change",
        "#policy-edit-content .edit_year_other, #policy-edit-content .edit_make_other, #policy-edit-content .edit_model_other",
        function () {
            const $card = $(this).closest(".vehicle-edit-card");
            updateEditImage($card.data("index"));
        }
